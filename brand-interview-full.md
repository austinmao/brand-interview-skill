...
Conducts a guided brand interview and assembles two complete documents — `BRAND.md` (strategy, positioning, audience, narrative, voice) and `DESIGN.md` (color, typography, spacing, and component tokens). Runs fully standalone in this Project/Custom-GPT/chat context. Automatically upgrades to a connected mode when the Glance brand connector's MCP tools are present (see **Mode Detection**) — no connector required to use this.
...
Before Phase 0, check your available tools for `brand_next_question`, `brand_submit_answer`, `brand_generate_candidates`, `brand_select_candidate`, `brand_finalize`, `brand_finalize_design` (or a combined `brand_interview` tool). These exist only when this account has the Glance brand connector installed and authorized.
...
Work through sections §0 → §10 in order. Use the "Brand Interview Guide" section below for per-section questions, field definitions, and the framework each field maps to.
...
Use the "BRAND.md Contract" and "BRAND.md Template" sections below. Populate every field from the interview and the selected candidate framing.
...
Defines the DESIGN.md schema this skill's Phase 5 must satisfy — matches the design-token frontmatter and 9-section format used by the platform's `design-md-generator` pipeline, so a DESIGN.md produced by this skill drops into the same downstream tooling as one produced by the real connector.
...
All 4 token groups (`colors`, `typography`, `rounded`, `spacing`) required and fully populated — no leftover `{{PLACEHOLDER}}` values. `components` may be an empty list when no site components exist yet.
...
Every role in the `colors:` frontmatter block gets one line in **Color Palette & Roles**, in this exact format:

```
- **{role}**: `{role}` — `{hex value}`
```

e.g. `- **background**: \`background\` — \`#FBFBFC\``

## Invariants

- Every color/typography/radius/spacing value named in the body prose MUST match a value in the frontmatter — never introduce a color or font not declared there
- `tokens.css` (produced alongside DESIGN.md in Phase 5) maps every frontmatter value to a `--site-*` custom property — CSS selectors reference `var(--site-*)`, never raw hex
- Re-assembling with the same Phase 4 winning direction produces identical output
...
Reference for Phase 5. Use this together with the DESIGN.md Contract above.
